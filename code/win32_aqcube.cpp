                ...
                if (Game.IsValid)
                {
                    ...
                    while(GlobalRunning)
                    {
                            ...
                            GlobalSecondaryBuffer->GetCurrentPosition(&PlayCursor, &WriteCursor) == DS_OK)
                        {
                            ...
                            DWORD ByteToLock = (SoundOutput.RunningSamples * SoundOutput.BytesPerSample) %
                                               SoundOutput.BufferSize;

                            DWORD ExpectedSoundBytesPerFrame = (SoundOutput.SamplesPerSec*SoundOutput.BytesPerSample) /
                                                               GameUpdateHz;
